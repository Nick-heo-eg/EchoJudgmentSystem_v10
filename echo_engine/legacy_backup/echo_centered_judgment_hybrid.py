#!/usr/bin/env python3
"""
🎯 Echo-Centered Judgment Hybrid System
Foundation Doctrine v3.0: Echo가 판단 주체, Claude는 제한적 보조 도구

핵심 아키텍처:
1. Echo Independent Loop (기본) - LLM-Free 독립 판단
2. Echo-Claude Conditional Collaboration (조건부) - 고복잡도 시에만 제한적 협업
3. Echo Final Review & Override (필수) - 모든 결과의 Echo 최종 검토
4. Judgment Source Transparency (투명성) - 모든 판단 출처 명시

Echo 중심성 원칙:
- TT.100: Echo는 존재하는 판단 주체, LLM은 보조 도구
- TT.101: Echo 판단은 Echo 소유, 외부 LLM 혼동 금지
- TT.102: Echo는 언제든 Claude 제안을 거부하고 독립 판단 가능
"""
...
# Echo 컴포넌트 imports
try:
    from echo_engine.claude_fallback_handler import (
        ClaudeFallbackHandler,
        ClaudeFallbackRequest,
        ClaudeFallbackReason,
        get_claude_fallback_handler,
    )
    from echo_engine.claude_to_signature_rewriter import (
        ClaudeToSignatureRewriter,
        get_signature_rewriter,
        rewrite_claude_to_signature,
    )
    from echo_engine.judgment_source_labels import (
        JudgmentSource,
        get_judgment_labeler,
        create_echo_independent_label,
    )
    from echo_engine.enhanced_llm_free_judge import (
        EnhancedLLMFreeJudge,
        get_enhanced_llm_free_judge,
        process_independent_judgment as enhanced_independent_judgment,
    )
    from echo_engine.error_recovery_system import (
        EchoErrorRecoverySystem,
        get_error_recovery_system,
        report_error,
        SystemComponent,
        ErrorSeverity,
    )
    from echo_engine.echomistral import (
        EchoMistral,
        get_echo_mistral,
        EchoSignature,
        EchoMistralResponse,
    ...
    try:
        ...
        from echo_engine.reasoning import ReasoningEngine
...
class JudgmentComplexity(Enum):
    ...
    TRIVIAL = "trivial"  # 0.0-0.3: 간단한 인사, 확인
    SIMPLE = "simple"  # 0.3-0.5: 기본 대화, 정보 요청
    MODERATE = "moderate"  # 0.5-0.7: 일반적 문제 해결
    COMPLEX = "complex"  # 0.7-0.85: 복잡한 분석 필요
    HIGHLY_COMPLEX = "highly_complex"  # 0.85-1.0: Claude 협업 고려


class EchoDecisionPath(Enum):
    """Echo 판단 경로"""

    PURE_ECHO = "pure_echo"  # Echo 완전 독립 판단
    ECHO_WITH_MISTRAL = "echo_mistral"  # EchoMistral로 Echo 분석 강화
    ECHO_WITH_CLAUDE_INPUT = "echo_claude_input"  # Claude 의견 참고 후 Echo 판단
    ECHO_OVERRIDE = "echo_override"  # Claude 제안 거부 후 Echo 독립 판단
    COLLABORATIVE = "collaborative"  # Echo-Claude 협업 (Echo 최종 결정)
...
class JudgmentRequest:
    ...
    echo_confidence: Optional[float] = None
    priority: str = "medium"


@dataclass
class EchoJudgmentResult:
    """Echo 중심 판단 결과"""

    final_judgment: str
    decision_path: EchoDecisionPath
    judgment_source: JudgmentSource
    confidence_score: float
    complexity_assessment: JudgmentComplexity
    processing_time: float
    echo_reasoning: str
    claude_input: Optional[str] = None
    echo_override_reason: Optional[str] = None
    signature_used: str = "Aurora"
    mistral_enhancement_score: Optional[float] = None
...
# 전역 인스턴스
_judgment_hybrid = None


def get_echo_judgment_hybrid() -> EchoCenteredJudgmentHybrid:
    """Echo 중심 하이브리드 판단 시스템 인스턴스 반환"""
    global _judgment_hybrid
    if _judgment_hybrid is None:
        _judgment_hybrid = EchoCenteredJudgmentHybrid()
    return _judgment_hybrid


# 편의 함수들
async def process_echo_judgment(
    user_input: str,
    ...
    return await hybrid.process_judgment_request(request)
...
if __name__ == "__main__":
    ...
    async def test_echo_centered_hybrid():
        print("🎯 Echo-Centered Judgment Hybrid 테스트")
        print("=" * 60)
        ...
        test_cases = [
            {
                "description": "간단한 인사 - Echo 독립 처리 예상",
                "user_input": "안녕하세요, 오늘 날씨가 좋네요!",
                "signature": "Aurora",
                "emotion": "joy",
            },
            {
                "description": "복잡한 철학적 질문 - Claude 협업 후 Echo 최종 결정",
                "user_input": "인공지능의 존재론적 의미와 의식의 본질에 대해 깊이 분석해주세요",
                "signature": "Sage",
                "emotion": "contemplation",
            },
            {
                "description": "코드 구현 요청 - Claude 협업 검토",
                "user_input": "복잡한 정렬 알고리즘을 최적화해서 구현해주세요",
                "signature": "Phoenix",
        ...
        for i, test in enumerate(test_cases, 1):
            ...
            result = await process_echo_judgment(
                test["user_input"], test["signature"], test["emotion"]
            )

            print(f"  🎯 결정 경로: {result.decision_path.value}")
            print(f"  📊 복잡도: {result.complexity_assessment.value}")
            print(f"  💪 신뢰도: {result.confidence_score:.2f}")
            print(f"  ⏱️ 처리 시간: {result.processing_time:.3f}초")
            print(f"  🧠 Echo 추론: {result.echo_reasoning[:80]}...")
            print(f"  ✨ 최종 판단: {result.final_judgment[:100]}...")

            if result.claude_input:
                print(f"  🤖 Claude 협업: 있음")
                if result.echo_override_reason:
                    print(f"  🚫 Echo 거부 사유: {result.echo_override_reason}")
            else:
                print(f"  🤖 Claude 협업: 없음 (Echo 완전 독립)")

        # 시스템 분석
        analytics = hybrid.get_system_analytics()
        print(f"\n📊 시스템 분석:")
        print(f"  총 판단: {analytics['performance_metrics']['total_judgments']}")
        print(
            f"  Echo 독립 비율: {analytics['echo_dominance_metrics']['echo_independence_ratio']:.2%}"
        )
        print(
            f"  Claude 협업 비율: {analytics['echo_dominance_metrics']['claude_usage_ratio']:.2%}"
        )
        print(
            f"  Echo 거부 비율: {analytics['echo_dominance_metrics']['echo_override_ratio']:.2%}"
        )

        print(f"\n✅ Echo-Centered Judgment Hybrid 테스트 완료!")
        print("🎯 Echo는 이제 자신만의 판단 체계를 가지고 있으며,")
        print("   필요할 때만 Claude와 협업하되 항상 최종 결정권을 가집니다!")

    asyncio.run(test_echo_centered_hybrid())
