"""
Adaptive Learning Memory - GPT-5 Level Dynamic Memory System
===========================================================

Implements advanced adaptive memory with:
- Context-aware memory formation and retrieval
- Experience-based pattern learning
- Dynamic memory consolidation and forgetting
...
class MemoryType(Enum):
    ...
    EPISODIC = "episodic"  # 에피소드 기억 (경험)
    SEMANTIC = "semantic"  # 의미 기억 (지식)
    PROCEDURAL = "procedural"  # 절차 기억 (방법)
    WORKING = "working"  # 작업 기억 (임시)
    META = "meta"  # 메타 기억 (학습에 대한 학습)


class MemoryImportance(Enum):
    """메모리 중요도"""

    CRITICAL = 5  # 절대 잊으면 안 되는 핵심 기억
    HIGH = 4  # 매우 중요한 기억
    MEDIUM = 3  # 중요한 기억
    LOW = 2  # 보통 기억
    MINIMAL = 1  # 낮은 중요도
...
class MemoryTrace:
    ...
    # 성과 연결
    success_correlation: float = 0.0  # -1 to 1, 성공과의 연관성
...
class LearningPattern:
    ...
    pattern_id: str
    pattern_type: str  # "success", "failure", "strategy", "context"

    # 패턴 정의
    pattern_signature: Dict[str, Any]  # 패턴을 식별하는 특성
    pattern_conditions: List[str]  # 패턴이 발생하는 조건
    ...
    # 전이 가능성
    transferability: float = 0.5  # 다른 상황으로의 전이 가능성
    generalization_level: float = 0.3  # 일반화 수준
...
class ConsolidationEvent:
    ...
    # 통합 유형
    consolidation_type: str  # "strengthen", "weaken", "merge", "split", "transfer"
    ...
    # 통합 결과
    consolidation_outcome: Dict[str, Any]
